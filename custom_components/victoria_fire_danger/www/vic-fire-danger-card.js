// Safari / Home Assistant frontend compatibility shim
// Some HA integration pages assume this symbol exists.
// Chromium tolerates it; WebKit (iOS) does not.
...
class VicFireDangerCard extends HTMLElement {
  ...
  set hass(hass) {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        :host { display: block; min-height: 485px; contain: layout; }
        ha-card {
          display: flex;
        ...
        .header-date { font-size: 14px; margin-bottom: 20px; color: var(--secondary-text-color); }
        .gauge-container { width: 260px; height: 140px; margin: 0 auto; position: relative; }
        .needle-pivot { transition: transform 1.5s ease-in-out; transform-origin: 130px 130px; transform: rotate(${needleAngle}deg); }
        .rating-box { background: ${ratingInfo.color}; color: #212121; padding: 8px 30px; font-size: 24px; font-weight: 900; border: 2px solid #212121; margin: 16px auto; }
        .rating-message { font-size: 18px; font-weight: 700; margin: 16px 0; }
        .ban-section { display: ${hasBan ? 'flex' : 'none'}; align-items: center; justify-content: center; margin-top: 10px; }
        .ban-label { font-size: 20px; font-weight: 800; color: #DB4433; margin-left: 12px; }
        .spacer { flex-grow: 1; }
        .forecast-grid { display: flex; gap: 8px; margin-top: 24px; padding-top: 16px; border-top: 1px solid var(--divider-color); }
        .forecast-item { flex: 1; background: var(--secondary-background-color); border-radius: 10px; padding: 10px 4px; border: 1px solid var(--divider-color); }
        .fc-day { font-size: 11px; font-weight: 700; margin-bottom: 6px; }
        .fc-rating-tag { font-weight: 900; padding: 4px 0; border-radius: 4px; width: 90%; margin: auto; font-size: 10px; }
        .fc-ban-badge { font-size: 8px; font-weight: 950; margin-top: 4px; opacity: 0.3; }
        .fc-ban-badge.active { opacity: 1; color: #ce161e; }
      </style>
      <ha-card>
        ...
        <div class="gauge-container">
          <svg viewBox="0 0 260 140">
            <path d="M 30 130 A 100 100 0 0 1 59.3 59.3 L 130 130 Z" fill="${COLORS.MODERATE}"/>
            <path d="M 59.3 59.3 A 100 100 0 0 1 130 30 L 130 130 Z" fill="${COLORS.HIGH}"/>
            <path d="M 130 30 A 100 100 0 0 1 200.7 59.3 L 130 130 Z" fill="${COLORS.EXTREME}"/>
            <path d="M 200.7 59.3 A 100 100 0 0 1 230 130 L 130 130 Z" fill="${COLORS.CATASTROPHIC}"/>
            <g class="needle-pivot">
              <path d="M 130 130 L 126 120 L 130 40 L 134 120 Z" fill="var(--primary-text-color)"/>
              <circle cx="130" cy="130" r="10" fill="var(--primary-text-color)"/>
            </g>
          </svg>
        </div>
  ...
  getNeedleAngle(rating) {
    return { MODERATE: -67.5, HIGH: -22.5, EXTREME: 22.5, CATASTROPHIC: 67.5 }[rating] ?? -90;
  }
  ...
  getRatingInfo(rating) {
    ...
    }[rating] ?? { color: '#ffffff', message: 'Check local conditions' };
  }
}
...
console.info(
  '%c VIC-FIRE-DANGER-CARD %c Version 1.1.0 ',
